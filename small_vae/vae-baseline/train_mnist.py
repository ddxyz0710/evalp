...
def parse_args():
    ...
    parser.add_argument('--fid_dataset', default='mnist_50k',  help='SVHN|cifar10 | celeba64 | lsun | imagenet | folder | lfw | fake')
    ...
    parser.add_argument('--model_name', type=str, default='STD-VAE', help='model name')
    ...
    parser.add_argument('--embedding_weight', type=float, default=0.312, help='weight of embedding loss')
    ...
    # -----------------------------------------------------------
    parser.add_argument('--lrG', type=float, default=1e-4, help='learning rate for G, default=0.0002')
    parser.add_argument('--lrI', type=float, default=1e-4, help='learning rate for I, default=0.0002')
    ...
    parser.add_argument('--lr_factor', default=0.99, type=float, help="lr multiplication factor in scheduler")
...
def get_output_dir(exp_id, opt, fs_prefix='./'):
    t = f"embwt{opt.embedding_weight}-lrI{opt.lrI}-lrG{opt.lrG}-wtdecay{opt.weight_decay}-lrf{opt.lr_factor}-epoch{opt.niter}"  #str(opt.datetime) #datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    if opt.mode == 'debug':
        ...
        output_dir = os.path.join(fs_prefix + 'output/' + exp_id, t, 
                                  f"seed-{opt.manualSeed}") # + '_' + str(opt.embedding_weight) + '_' + str(opt.manualSeed))
...
# share m and s
    ...
    if not os.path.exists(path):
        print(f"No FID stats found at {path}.")
        print(f'Computing fid stats of the train data with {total_fid_samples} samples ...')
...
def train(opt, output_dir):
    ...
    # def kl_loss(y_true, y_pred):
        ...
        kl = 0.5 * torch.sum(torch.exp(2*log_sigma) + torch.square(mu) - 1. - 2*log_sigma, axis=1) 
        # kl = 0.5 * torch.sum(torch.exp(log_sigma) + torch.square(mu) - 1. - log_sigma, dim=1) # This line has bug since we prredict log_sigma not log_var
        return kl
    ...
    def reparametrize(mu, log_sigma, is_train=True):
        if is_train:
            #std = log_sigma.exp_()  # in-place op; otential bug. Chnages log_sigma outside this fn
            std = log_sigma.exp() 
            # std = torch.exp(log_sigma.mul(0.5)) # This line has potential bugs cause it changes the log_sigma
    ...
    def eval_model(opt, model_type='bestmse'):
        ...
        with torch.no_grad():
            ...
            fid_dict = get_fid_vae(netI, netG, opt, opt.num_last_epoch_fid_samples, recon_fid=True)
    ...
    def get_fid_vae(netI, netG, opt, total_fid_samples, recon_fid=False):
        '''
        model: decoder of vae
        #! using test dataloader to calculate FID against
        '''
    ...
    optimizerG = torch.optim.Adam(netG.parameters(), lr=opt.lrG, weight_decay=opt.weight_decay, betas=(0.9, 0.999))
    if opt.netG != '':
    ...
    optimizerI = torch.optim.Adam(netI.parameters(), lr=opt.lrI, weight_decay=opt.weight_decay, betas=(0.9, 0.999))
    if opt.netI != '':
    ...
    lrG_schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizerG, factor=opt.lr_factor, patience=5)
    lrI_schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizerI, factor=opt.lr_factor, patience=5)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            inputV = Variable(input)
            # inputV = (inputV + 1)/2.0
            #Todod: Input to netI should be in [0, 1]
        ...
        with torch.no_grad():
            ...
            if ((epoch) % opt.fidIter) == 0:
                # Run fid calculation to precopute statistics if not present
                ref_fid_stats_path = os.path.join(opt.fid_dir, opt.fid_dataset + '.npz')
                if not os.path.exists(ref_fid_stats_path):
                    fid_dict = get_fid_vae(netI, netG, opt, opt.num_last_epoch_fid_samples, recon_fid=True)
        ...
        if (epoch+1) % opt.diagIter == 0 or epoch == 0 or epoch + 1==opt.niter:
            ...
            stat_1['norm(grad(I))'].append(stats_values['norm(grad(I))']) #netIgradNorm.data.item())
            stat_1['norm(grad(G))'].append(stats_values['norm(grad(G))'])
            # stat_1['norm(grad(E))'].append(netEgradNorm.data.item())
            stat_1['norm(weight(I))'].append(stats_values['norm(weight(I))']) #netIpNorm.data.item())
...
def main():
    ...
    exp_id = os.path.join(data_exp, opt.exp_name)
