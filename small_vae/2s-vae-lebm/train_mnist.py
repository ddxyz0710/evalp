...
def parse_args():
    ...
    parser.add_argument('--fid_dataset', default='mnist_10k',  help='SVHN|cifar10 | celeba64 | lsun | imagenet | folder | lfw | fake')
    ...
    parser.add_argument('--model_name', type=str, default='RAE-Baseline', help='model name')
    ...
    parser.add_argument('--bottleneck_factor', type=int, default=16, help='latent embedding size')
    ...
    parser.add_argument('--weight_decay', type=float, default=0., help='weight decay for n/ws')
    parser.add_argument('--lr_factor', type=float, default=0.8, help='LR multiplier in scheduler (must be < 1.0)')
    ...
    parser.add_argument('--nsblocks', type=int, default=2, help='no of blocks in flow-sampler')
    parser.add_argument('--hidden_dimF', type=int, default=128, help='Hidden dim in coupling layers')
...
def get_output_dir(exp_id, opt, fs_prefix='./'):
    ...
    t = f"nblk{opt.nsblocks}-hdimf{opt.hidden_dimF}-ndf{opt.ndf}-klwt{opt.kl_weight}-lrE{opt.lrE}-lrF{opt.lrF}-wtdecay{opt.weight_decay}-lrf{opt.lr_factor}-epoch{opt.niter}" 
    if opt.mode == 'debug':
        output_dir = os.path.join(fs_prefix + 'output/' +  exp_id, 'debug_' + t,
                                  f"seed-{opt.manualSeed}")
    else:
...
def get_cifar_dataset(opt):
    ...
    train_dataset = CIFAR10NoLabels(root='./data/cifar', train=True, download=True, transform=transform)
    test_dataset = CIFAR10NoLabels(root='./data/cifar', train=False, download=True, transform=transform)
...
def get_mnist_dataset(opt):

    class MNIST(datasets.MNIST):
        def __getitem__(self, index):
            img, _ = super(MNIST, self).__getitem__(index)
            return img


    transform = transforms.Compose([
        # transforms.Resize(opt.imageSize),  # Resize images to desired size
        transforms.Pad(2, fill=0), # pad 2 rows and columns
        transforms.ToTensor(),             # Convert PIL images to tensors
        # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # Normalize images
    ])

    # CIFAR-10 dataset
    # train_dataset = MNISTNoLabels(root='./data/', train=True, download=True, transform=transform)
    # test_dataset = MNISTNoLabels(root='./data/', train=False, download=True, transform=transform)
...
class Decoder(nn.Module):
    def __init__(self, opt):
        ...
        layers.append(nn.Sequential(nn.Linear(opt.bottleneck_factor, 8*self.nf * 8 * 8)))

        layers.append(
            nn.Sequential(
                nn.ConvTranspose2d(8*self.nf, 4*self.nf, 4, 2, padding=2, output_padding=1, bias=False),
                nn.BatchNorm2d(4*self.nf, momentum=self.bn_mom, eps=self.bn_eps),
                nn.ReLU(),
            )
        )
        ...
        layers.append(
            nn.Sequential(
                nn.ConvTranspose2d(4*self.nf, 2*self.nf, 4, 2, padding=0, output_padding=1, bias=False),
                nn.BatchNorm2d(2*self.nf, momentum=self.bn_mom, eps=self.bn_eps),
                nn.ReLU(),
            )
        )
        ...
        # layers.append(
        #     nn.Sequential(
        #         nn.ConvTranspose2d(256, 128, 4, 2, padding=2, output_padding=1, bias=False),
        ...
        layers.append(
            nn.Sequential(
                nn.ConvTranspose2d(2*self.nf, self.n_channels, 4, 1, padding=2), 
                nn.Sigmoid()
            )
...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(self.n_channels, self.nf, 4, 2, padding=1),  # use kernel=5, pad=2?
                nn.BatchNorm2d(128, momentum=self.bn_mom, eps=self.bn_eps),
                nn.ReLU(),
            )
        )
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(self.nf, 2 * self.nf, 4, 2, padding=1), 
                nn.BatchNorm2d(2 * self.nf, momentum=self.bn_mom, eps=self.bn_eps), 
                nn.ReLU()
            )
        )
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(2*self.nf, 4*self.nf, 4, 2, padding=1), 
                nn.BatchNorm2d(4 * self.nf, momentum=self.bn_mom, eps=self.bn_eps), 
                nn.ReLU()
            )
        )
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2d(4*self.nf, 8*self.nf, 4, 2, padding=1), 
                nn.BatchNorm2d(8 * self.nf, momentum=self.bn_mom, eps=self.bn_eps), 
                nn.ReLU()
            )
        )
        ...
        self.embedding = nn.Linear(8 * self.nf * 2 * 2, opt.bottleneck_factor)
        self.log_sigma = nn.Linear(8 * self.nf * 2 * 2, opt.bottleneck_factor)
        self.tanh = nn.Tanh()

    # def forward(self, x: torch.Tensor, output_layer_levels: List[int] = None):
    def forward(self, x):

        x = 2 * x - 1 # This operation is needed if x in [0, 1] 

        max_depth = self.depth
        out = x  #here x should be in [-1, 1]
...
class _netE(nn.Module):
    def __init__(self, args):
        ...
        self.ebm = nn.Sequential(
            ...
            f,
            # apply_sn(nn.Linear(args.ndf, args.ndf)), #! Added one more layer
            # f,
...
def train(opt, output_dir):
    ...
    def diag_normal_NLL(z, z_mu, z_log_sigma):
        ...
        #TODO(Warning): Potentioal bug. Check whether z_log_sigma is intended as variance or std.
    ...
    def eval_model(opt):
        ...
        netG = Decoder(opt)
        netG_path = opt.netG
        ckpt = torch.load(netG_path, map_location='cpu')
        netG.load_state_dict(ckpt['model_state'])  
        if opt.cuda:
            netG.cuda()

        netF = load_flows.get_inv_model(num_inputs=nz, num_hidden=opt.hidden_dimF, num_blocks=opt.nsblocks, flow='realnvp')
        netF_path = '%s/%s_%s.pth' % (output_dir, 'netF', 'bestfid')
        ckpt = torch.load(netF_path, map_location='cpu')
        ...
        with torch.no_grad():
            ...
            print(f"Calculating FID with {opt.num_last_epoch_fid_samples} samples ... ")
            fid_dict = get_fid_vae(netI, ext_netG, opt, opt.num_last_epoch_fid_samples, recon_fid=True)
    ...
    dataloader, dataset_full, test_dataloader = get_mnist_dataset(opt)
    ...
    #optimizerG = torch.optim.Adam(netG.parameters(), lr=1e-3, weight_decay=0, betas=(0.9, 0.999))
    if opt.netG != '':
        ckpt = torch.load(opt.netG, map_location='cpu')
        netG.load_state_dict(ckpt['model_state'])
    ...
    #optimizerI = torch.optim.Adam(netI.parameters(), lr=1e-3, weight_decay=0, betas=(0.9, 0.999))
    if opt.netI != '':
        ckpt = torch.load(opt.netI, map_location='cpu')
    ...
    optimizerE = torch.optim.Adam(netE.parameters(), lr=opt.lrE, weight_decay=opt.weight_decay, betas=(0.5, 0.999))
    # optimizerE = torch.optim.RMSprop(netE.parameters(), lr=opt.lrE) #weight_decay=1e-4, betas=(0.5, 0.9))
    if opt.netE != '':
        ckpt = torch.load(opt.netE, map_location='cpu')
    ...
    ## Sampler model
    netF = load_flows.get_inv_model(num_inputs=nz, num_hidden=opt.hidden_dimF, num_blocks=opt.nsblocks, flow='realnvp')
    optimizerF = torch.optim.Adam(netF.parameters(), lr=opt.lrF, weight_decay=opt.weight_decay, betas=(0.5, 0.999))
    # optimizerF = torch.optim.RMSprop(netF.parameters(), lr=opt.lrF) #weight_decay=1e-4)
    if opt.netF != '':
        ckpt = torch.load(opt.netF, map_location='cpu')
        netF.load_state_dict(ckpt['model_state'])
    ...
    lrF_schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizerF, factor=opt.lr_factor, patience=5)
    lrE_schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizerE, factor=opt.lr_factor, patience=5)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            z_pos_mu, z_pos_log_sigma = netI(inputV)  # <-- will be used later
            z_pos = reparametrize(z_pos_mu, z_pos_log_sigma) # <-- will be used later
            ...
            # for fstep in range(opt.fsteps):
                ...
                # # z_neg = netF.sample(noise=pnoise) # <-- used later
                # # z_neg = netF(pnoise)

                # log-prob calculation, see: eq(1) of real-nvp paper. 
                z_neg, logdets = netF(pnoise, mode='direct')
                # log_probF = (-0.5 * pnoise.pow(2) - 0.5 * math.log(2 * math.pi)).sum(
                #         -1, keepdim=True)
                neg_log_prob_z = diag_standard_normal_NLL(pnoise)
                log_probF = torch.mean(-neg_log_prob_z - logdets)
                # # log_probF = torch.mean(netF.log_probs(z_neg)) # scalar # detach z_neg?
                neg_log_p0_z_neg = torch.mean(diag_standard_normal_NLL(z_neg.detach()))
                # # # reverse_klF = torch.mean(log_probF) + torch.mean(diag_standard_normal_NLL(z_neg))
    ...
    fid_dict = get_fid_vae(netI, ext_netG, opt, opt.num_last_epoch_fid_samples, recon_fid=False)
    opt_dict = {'netE': (netE, optimizerE),'netF': (netF, optimizerF)}
