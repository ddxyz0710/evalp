...
class BatchNormFlow(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        #! Edit: Switching the mode; since we are using sample method (or inverse mode) before. 
        # Running means and vars now have to be calculated duirng inverse mode.  
        # Similar issue at: https://github.com/ikostrikov/pytorch-flows/issues/8
        # if mode == 'direct':
        if mode == 'inverse': 
            if self.training:
...
class CouplingLayer(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, mode='direct'):
        mask = self.mask
        mask = mask.to(inputs.device) #! Added to remove device mismatch
