...
from models import RealNVP, RealNVPLoss, RealNVP_Big #TODO:  rename the util folder; currently there are two
...
def main(eval_args):
    ...
    if eval_args.is_local:
        if args.dataset == 'mnist':
            ...
            args.data = eval_args.data_dir   
            print(eval_args.data_dir)             
    ...
    #! REalNVP is made small. Revert changes if needed. 
    samplerNet = RealNVP_Big(num_scales=2, in_channels=in_channel, mid_channels=eval_args.sampler_mid_channels, 
...
def train(args, epoch,
    ...
    # if concat:
    #     in_channel = args.s_size[args.ftr_map_size] + args.Cin
    # else:
...
def test(args, epoch,
    ...
    evalp = EVALP(Fnet, list(fixed_noise.size()[1:]), out_size=[args.in_channel, args.ftr_map_size, args.ftr_map_size])
...
def create_generator_vae_with_evalp(model, evalp, batch_size, num_total_samples):
    ...
    for i in range(num_iters):
        with torch.no_grad():
            ...
            output = model.decoder_output(logits)
            #! Note: Changing to sampling using sample() fn to be consistent with NCP code. 
            output_img = output.mean if isinstance(output, torch.distributions.bernoulli.Bernoulli) else output.mean()
