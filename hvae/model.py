...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance):
        ...
        # self.q_z_x_samples = torch.Tensor()
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    # print('from q   ', z.size())
                    # print(z.size())
                    #sampling from posterior with trained model.
                    log_q_conv = dist.log_p(z)
                    # apply NF
                    for n in range(self.num_flows):
                    ...
                    log_p_conv = dist.log_p(z)
                    # zp, _ = dist.sample()
                    # print('from p   ', zp.size())

                    all_p.append(dist)
                    all_log_p.append(log_p_conv)

                # 'combiner_dec'
                s = cell(s, z) #blue plus
                idx_dec += 1
            else:
                s = cell(s)
    ...
    # Add sampleer for evalp
    def sample_with_evalp(self, evalp, num_samples, t, noise=None):
        ''' Ignores num_samples if noise is not None
        '''
        pz_dec_samples = []
        scale_ind = 0
        # z0_size = [num_samples] + self.z0_size
        # replace the normal samples with evalp samples

        if noise is not None:
            z = evalp.sample(noise=noise).cuda()
        else:
            z = evalp.sample(num_samples=num_samples).cuda()
        # if noise is not None:
        #     z = noise
        # else:
        #     z = torch.randn([num_samples]+evalp.size).cuda()
        # # if z.ndim <= 2:
        # #     z = z.view([z.shape[0]] + list(evalp.out_size))
        pz_dec_samples.append(z.detach())
        ...
        logits = self.image_conditional(s)
        # return logits, pz_dec_samples
        return logits
    
    def importance_sample(self, Dnet, num_samples, sir_multinomial_dim, t, noise=None):
        pz_dec_samples = []
        scale_ind = 0

        if noise is not None:
            raise ValueError("Custom noise not supported yet.")

        z0_size = [num_samples] + [sir_multinomial_dim] + self.z0_size # (n, md, 20, 8, 8) for celeba64
        dist = Normal(mu=torch.zeros(z0_size).cuda(), log_sigma=torch.zeros(z0_size).cuda(), temp=t)
        z, eps = dist.sample()
        s = self.prior_ftr0.unsqueeze(0)
        batch_size = z.size(0)
        s = s.expand(batch_size, -1, -1, -1)
        importance_weights = Dnet.get_importance_weights(z, s, dist, t)
        c = torch.distributions.categorical.Categorical(logits=importance_weights)
        
        selected_iw = c.sample()
        z = [z[i][idx] for i, idx in enumerate(selected_iw)]
        z = torch.stack(z)
        pz_dec_samples.append(z.detach())
    ...
    def importance_sample_with_evalp(self, Dnet, evalp,  num_samples, sir_multinomial_dim, t, noise=None):
        pz_dec_samples = []
        scale_ind = 0
        ...
        # dist = Normal(mu=torch.zeros(z0_size).cuda(), log_sigma=torch.zeros(z0_size).cuda(), temp=t)
        dist = evalp 
        # z, eps = dist.sample(num_samples = int(num_samples * sir_multinomial_dim))
        z = dist.sample(num_samples = int(num_samples * sir_multinomial_dim))
        z = z.view(z0_size)
    ...
    def get_last_latent(self, x):
        ...
        dist = Normal(mu_q, log_sig_q)   # for the first approx. posterior
        # z, eps = dist.sample()
        z = mu_q #! Change to mean for better stability?
        log_q_conv = dist.log_p(z)
        # apply normalizing flows
        nf_offset = 0
        for n in range(self.num_flows):
        ...
        z_p = dist.sample()[0].detach()
        # z_p = dist.sample_given_eps(eps).detach()
        
        pz_samples.append(z_p)
        ...
        return pz_samples[0], qzx_samples[0], s_samples[0]
    

    def decoder_output(self, logits):
        if self.dataset == 'mnist':
            return Bernoulli(logits=logits)
        elif self.dataset in {'cifar10', 'celeba_32', 'celeba_64', 'celeba_256', 'imagenet_32', 'imagenet_64', 'ffhq'}:
