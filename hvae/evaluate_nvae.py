...
def main(eval_args):
    ...
    logging = utils.Logger(eval_args.local_rank, eval_args.save, fname='eval_log.txt')
    ...
    if eval_args.eval_mode == 'evaluate':
        ...
        # set_bn(model, bn_eval_mode, num_samples=2, t=eval_args.temp, iter=500) #! changed this to match with the sampling temp for fid calculation
        logging.info("Adjusting batch norm with temp=1.0")
        set_bn(model, bn_eval_mode, num_samples=16, t=1.0, iter=500) #! Comment or uncomment based on need
        ...
        save_syn_dir = os.path.join(eval_args.save, 'syn')
