        ...
        if epoch % eval_freq == 0 or epoch == (args.epochs - 1):
            with torch.no_grad():
                ...
                for t in [0.7, 0.8, 0.9, 1.0]:
                    ...
                    output_tiled = output_tiled.cpu().numpy().transpose(1, 2, 0)
                    output_tiled = np.asarray(output_tiled * 255, dtype=np.uint8)
                    output_tiled = np.squeeze(output_tiled)
                    file_path = os.path.join(save_syn_dir, 'gpu_%d_samples_%d.npz' % (args.local_rank, epoch))
                    np.savez_compressed(file_path, samples=output_img.cpu().numpy())
                    file_path = os.path.join(save_syn_dir, 'gpu_%d_samples_%d.png' % (args.local_rank, epoch))
...
def train(train_queue, model, cnn_optimizer, grad_scalar, global_step, warmup_iters, writer, logging):
    ...
    for step, x in enumerate(train_queue):
        ...
        with autocast():
            ...
            if step % 100==0: logging.info("{}/{}    loss:{}".format(step, len(train_queue),loss.data.item()))
