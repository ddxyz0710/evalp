...
class Logger(object):
    def __init__(self, rank, save, fname=None):
        self.rank = rank
        if self.rank == 0:
            ...
            if fname is None:
                ...
                fh = logging.FileHandler(os.path.join(save, fname))
...
def tile_image_rectangle(batch_image, n):
    # assert n * n == batch_image.size(0)
    m = n//4
    channels, height, width = batch_image.size(1), batch_image.size(2), batch_image.size(3)
    batch_image = batch_image.view(m, 4, channels, height, width)
    batch_image = batch_image.permute(2, 0, 3, 1, 4)                              # n, height, n, width, c
    batch_image = batch_image.contiguous().view(channels, m * height, 4 * width)
    return batch_image
...
def num_output(dataset):
    if dataset == 'mnist':
        ...
        return 3 * 32 * 32
    elif dataset.startswith('celeba') or dataset.startswith('imagenet'):
...
if __name__ == '__main__':
    g = groups_per_scale(5, 16, is_adaptive=True, minimum_groups=4)
    print(kl_balancer_coeff(5, g, fun='square'))
    """
    import argparse
    import matplotlib
    ...
    parser.add_argument('--data', type=str, default='/tmp/nasvae',
    ...
    train_queue, valid_queue, num_classes = get_loaders(args, mode='search', dataset=None)
    batch = next(iter(train_queue))

    img = tile_image(batch[0], 8)
    plt.imshow(img.squeeze().data.numpy())
    plt.show()
    
    """
    print(groups_per_scale(5, 16, is_adaptive=True, divider=2, minimum_groups=1))

    ind = torch.LongTensor([[0, 1], [2, 3]]).cuda()
    y = onehot(ind, depth=4, dim=1)
    print(y[0, :, 0])
    print(y[0, :, 1])
    print(y[1, :, 0])
    print(y[1, :, 1])
